...
load(file = 'Data/Rdata/model-1-sim_data-CHRIST.Rdata')

set.seed(13)


# CUT CHRISTIANSHAVN to remove leading zeros
cut_day <- 14
I_it_est <- matrix(I_it_est[cut_day:length(I_it_est)])
S_it_est <- matrix(S_it_est[cut_day:length(S_it_est)])
I_it_daily <- I_it_daily[cut_day:length(I_it_daily)]
I_plus1 <- matrix(I_plus1[cut_day:length(I_plus1)])
S_plus1 <-  matrix(S_plus1[cut_day:length(S_plus1)])
Nsteps <- length(I_it_est)
...
loops <- 2000
R_i <- seq(from = 0, to = 0, length.out = length(I_it_est))
...
for (z in 1:loops){
  model_1_full[z] <- as.data.frame(rowSums(I_est_pe_list[[z]]))
}
...
for (z in 1:loops){
  model_1_tplus1[z] <- as.data.frame(rowSums(I_plus1_list[[z]]))
}
...
model_1_tplus1_plot <- ggplot() +
  ...
  theme(plot.title = element_text(size = 22, face="bold"),
        ...
        axis.title.x = element_text(size = 21, face = "bold"),
        axis.title.y = element_text(size = 21, face = "bold", vjust = 1.4))+
  ggtitle('Christianshavn daily infected\n 1-step-ahead simulated n = 2000')
model_1_tplus1_plot