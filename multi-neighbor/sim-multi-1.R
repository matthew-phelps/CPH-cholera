...
for (z in 1:loops){
  for (t in 1:(Nsteps-1)){
    for(i in 1:Nquarter){
      ...
      Lambda_quart[[i]][t, ] <- (S_it_est[t, i] / N_it[i]) * ( betas[, i] * I_sim_vect[t, ])
    }
  }
...
I_new_mat[[x]]
I_attr[[x]]
m <- matrix(colSums(I_new_mat[[x]], na.rm = T))
p <- matrix(colSums(I_attr[[x]]) )
m <- m[1:9]
p-m
p
colSums(Lambda_quart[[x]], na.rm = T)
...
for (z in 1:loops){
  for (t in 1:(Nsteps-1)){
    for(i in 1:Nquarter){
      ...
      Lambda_quart[[i]][t, ] <- (S_plus1_mat[t, i] / N_it[i]) * ( betas[, i] * I_sim_vect[t, ])
    }
  }
  # For each quarter: store sum of infections attributed to each quarter over
  # all time-steps
