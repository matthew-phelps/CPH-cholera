...
loops <- 4 # Has to be the same for both full sum and t+1 sim
duration <- 5 # In days. "1-2 weeks" from DOI:  10.1038/nrmicro2204
n_gam <- loops*Nsteps*Nquarter
gamma <-array(rexp(n_gam, 5), c(loops, Nsteps, Nquarter))
...
for (z in 1:loops){
  for (t in 1:(Nsteps-1)){
    for(i in 1:Nquarter){
      ...
      I_prev_vect[t + 1, i] <- max(0, (I_new[t, i] + I_prev_vect[t, i] - R_new[t, i]))
...
for (z in 1:loops){
  for (t in 1:(Nsteps-1)){
    for(i in 1:Nquarter){
      ...
      R_new[t, i] <- rpois(1, LambdaR[t, i])
      I_new[t, i] <- rpois(1, (Lambda_est_pe[t, i] ) )
      I_prev_vect[t + 1, i] <- max(0, (I_it_daily[t, i] + I_prev_vect[t, i] - R_new[t, i]))
...
I_prev_vect <-    matrix(nrow = Nsteps, ncol = Nquarter)
S_it_est <-       matrix(nrow = Nsteps, ncol = Nquarter)
I_7th_temp <-     array(NA, c(loops, Nsteps, Nquarter))
Lambda_quart <-   list(data.frame(matrix(nrow = Nsteps, ncol = Nquarter)))
I_new_mat <- list(data.frame(matrix(data = NA, nrow = Nsteps, ncol = Nquarter)))
# Attributable infections variables:
Lambda_quart<-lapply(1:Nquarter,
...
for (z in 1:loops){
  for (t in 1:(Nsteps-8)) {
    for (t7 in t:(t+7)){
      for(i in 1:Nquarter){
        if (t7 == t){
          # If it's the 1st time-step in 7-day prediction window, use "observed" incidence data
          Lambda_est_pe[t7, i] <- S_it_est[t7, i] / N_it[i] * sum( betas[, i] * I_prev_vect[t7, ])
          LambdaR[t7, i] <- I_prev_vect[t7, i] * gamma
          R_new[t7, i] <- rpois(1, LambdaR[t7, i])
          I_new[t7, i] <- rpois(1, (Lambda_est_pe[t7, i] ) )
          I_prev_vect[t7 + 1, i] <- max(0, (I_it_daily[t, i] + I_prev_vect[t7, i] - R_new[t7, i]))
          S_temp <- (S_it_est[t7, i]) -    (I_new[t7, i]) / (phi)
          S_it_est[t7 + 1, i] <- max(0, S_temp)
          Lambda_quart[[i]][t7, ] <- (S_it_est[t7, i] / N_it[i]) * ( betas[, i] * I_prev_vect[t7, ])
        } else {
          Lambda_est_pe[t7, i] <- S_it_est[t7, i] / N_it[i] * sum( betas[, i] * I_prev_vect[t7, ])
          LambdaR[t7, i] <- I_prev_vect[t7, i] * gamma
          R_new[t7, i] <- rpois(1, LambdaR[t7, i])
          I_new[t7, i] <- rpois(1, (Lambda_est_pe[t7, i] ) )
          I_prev_vect[t7 + 1, i] <- max(0, (I_new[t7, i] + I_prev_vect[t7, i] - R_new[t7, i]))
          S_temp <- (S_it_est[t7, i]) -    (I_new[t7, i]) / (phi)
          S_it_est[t7 + 1, i] <- max(0, S_temp)
          Lambda_quart[[i]][t7, ] <- (S_it_est[t7, i] / N_it[i]) * ( betas[, i] * I_prev_vect[t7, ])
        }
    ...
    # Store new infections at the 7th time-step for each quarter
    I_7th_temp[z, (t+7), ] <- I_new[t7, ]
    
    # For each quarter: store sum of infections attributed to each quarter over
    # # all time-steps
    # I_attr[[z]] <- sapply(Lambda_quart, colSums, na.rm=T)
    # # Store new infections at the 7th time-step for each quarter for each simulation
    # I_new_mat[[z]] <- (I_new[20])
    # I_new_mat[[z]]$sim_num <- z
  } 
  browser()
}
...
i = 9
max(0, (I_it_daily[t, i] + I_prev_vect[t7, i] - R_new[t7, i]))

# T + 7 : DATA RESHAPE --------------------------------------------------------------

# Simulated daily incidence to long format:
y <- do.call(rbind.data.frame, I_new_plus7) # merge all sims to 1 df
y[is.na(y)] <- 0
colnames(y) <- c(q_names, "sim_num")
y$day <- 1:112
I_simulated_plus1 <- gather(y, quarter, I_simulated, 1:9) # wide to long
