# Model 1 - Only allow internal, water, and shared border to carry infection
...
model {
  ...
  for (i in 1:Nquarter){
    ...
    for (j in 1:Nquarter){
      ...
      # If is a water-pipe connection b/w neighborhoods, foi = beta_2, else 0
      # Can't be exactly 0 otherwise no way for infection to get to new quarters
      # and JAGS won't run
      foi[i, j] <- ifelse(water_OR_border[i, j]==1, beta[i, j], 1.0e-9);
    } 
  }
