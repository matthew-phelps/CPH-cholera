...
pander(x, big.mark=",", split.cells = 10)
```
...
dic1 <- DIC_list %>%
  summaryDic() %>%
  data.frame() %>%
  dplyr::select(-8, -9) %>% # We don't use model 6 and 7 anymore
  `colnames<-` (mod_names)
...
## Figure 4  
Median of the log of the transmission coefficiants from model 5 (fully saturated model)
