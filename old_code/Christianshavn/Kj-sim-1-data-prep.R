...
load(file = "Data\\Rdata\\Kj-model-1-dataList.Rdata")
...
I_i_t1[1, 1] <- (combined$sick.total.week[which(combined$quarter== "Kjoebmager")])[1]
S_i_t1[1, 1] <- (combined$S[which(combined$quarter== "Kjoebmager")])[1]

for (t in 1:Nsteps){
  N_it[1, t] <- (combined$est.pop.1853[which(combined$quarter== "Kjoebmager")])[t]
}
...
save(list = ls(), file = 'data/Rdata/Kj-model-1-sim_data.Rdata' )
