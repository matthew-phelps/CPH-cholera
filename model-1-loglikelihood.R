...
I_fake_plus1_phi_60 <- I_fake_plus1_phi[, 20:65]


#Separate for-loop for phi-vector since it is different structure
...
# LL for fake phi
ll_t <- matrix(NA, nrow = num_loops, ncol = ncol(I_fake_plus1_phi_60))
ll_sim_sum <- vector(length = num_loops)
ll_t <- dpois(I_incidence_60, I_fake_plus1_phi_60, log = T)
ll_sim_sum <- rowSums(ll_t)

which.min(ll_sim_sum)
I_fake_plus1_phi[106,]
boxplot(ll_sim_sum)


# LL for fitted phi:
ll_t <- matrix(NA, nrow = num_loops, ncol = ncol(I_fit_plus1_phi_60))
ll_sim_sum <- vector(length = num_loops)
ll_t <- dpois(I_incidence_60, I_fit_plus1_phi_60, log = T)
ll_sim_sum <- rowSums(ll_t)
boxplot(ll_sim_sum)
